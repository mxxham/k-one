...
export default function PutawayScanPage() {
  ...
  const loadTasks = useCallback(async () => {
    try {
      ...
      if (selected) {
        ...
        setSelected(fresh);
        if (fresh) setCurrent(fresh.rows.find((r) => r.status !== 'Confirmed') ?? null);
  ...
  const openTask = (task: MyTask) => {
    setSelected(task);
    setCurrent(task.rows.find((r) => r.status !== 'Confirmed') ?? null);
    setStep('lpn');
    setMismatch(null);
    setOverrideReason('');
    setLastMatched(null);
  ...
  const confirmPallet = async () => {
    ...
    try {
      ...
      const nextRow = selected.rows.find((r) => r.status !== 'Confirmed' && r.id !== current.id) ?? null;
      setCurrent(nextRow);
      setStep('lpn');
      setMismatch(null);
      setOverrideReason('');
  ...
  const doneCount = selected?.rows.filter((r) => r.status === 'Confirmed').length ?? 0;
        ...
        <Card title="Task untuk checklist partner">
          {loading ? (
            ...
            <ul className="divide-y divide-gray-100">
              {tasks.map((t) => {
                const d = t.rows.filter((r) => r.status === 'Confirmed').length;
                return (
                  <li key={t.id}>
