...
export default function PutawayTasksPage() {
  ...
  const loadList = useCallback(async () => {
    try {
      ...
      const mapped = (res.rows || []).map((r: any) => ({
        ...r,
        pallet_count: r.item_count,
        done_count: r.confirmed_count,
      }));
      setRows(mapped as TaskRow[]);
    } catch (err: any) {
  ...
  const openDetail = async (id: number) => {
    try {
      ...
      setDetail({ task: res.task, rows: (res.items || []) as TaskItem[] });
    } catch (err: any) {
  ...
  const doneRows = detail?.rows.filter((r) => r.status === 'Confirmed').length ?? 0;
  const editable = task && (task.status === 'Open' || task.status === 'Pending' || task.status === 'In Progress');
