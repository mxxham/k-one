...
class WebGLErrorBoundary extends Component<{ children: React.ReactNode }, { error: string | null }> {
    ...
    if (this.state.error) {
      return (
        <div className="p-4 bg-red-50 border border-red-200 rounded-xl text-sm text-red-700">
          <div className="font-bold mb-1">3D rendering error:</div>
...
const AISLES = ['CA', 'CB', 'CC', 'CD', 'CE', 'STG'];
...
export default function RackViews({ tab }: { tab: 'rackmap' | 'rack3d' }) {
  ...
  const loadBins = useCallback(async () => {
    try {
      ...
      const data = (res.rows || []) as Bin3D[];
      console.log('[Rack3D] loaded bins:', data.length);
      setBins(data);
    } catch (err: any) {
