...
/** Superset interface covering both inbound and outbound product fields. */
...
export interface ProductSearchProps {
  /** API module to call: 'inbound' or 'outbound' — used when `endpoint` is not provided */
  apiModule?: 'inbound' | 'outbound';
  /** Full API endpoint path (e.g. 'inbound/search_products') — overrides `apiModule` when provided */
  endpoint?: string;
...
export default function ProductSearch({
  ...
  useEffect(() => {
    ...
    timer.current = setTimeout(async () => {
      setSearching(true);
      try {
        const mod = endpoint ? endpoint.split('/')[0] : apiModule;
        const act = endpoint ? endpoint.split('/')[1] || 'search_products' : 'search_products';
        const res = await api(mod, act, { params: { q } });
