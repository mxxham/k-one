...
-- Show current state
SELECT 'BEFORE' as status, COUNT(*) as total_cc FROM location_master WHERE aisle = 'CC';
SELECT 'BEFORE' as status, rack FROM location_master WHERE aisle = 'CC' ORDER BY rack;

-- Step 1: Delete incorrectly numbered CC Side B entries
-- CC35-CC40 (wrong numbering) should be deleted; CC18-CC34 are correct Side B
DELETE FROM location_master WHERE aisle = 'CC' AND rack BETWEEN '35' AND '40';

-- Step 2: Insert correct CC18-CC34 (17 bays × 5 levels × 2 positions = 170 rows)
-- Each bay has 10 rows: A01 A02 B01 B02 C01 C02 D01 D02 E01 E02
...
-- Show after state
SELECT 'AFTER' as status, COUNT(*) as total_cc FROM location_master WHERE aisle = 'CC';
SELECT 'AFTER' as status, rack FROM location_master WHERE aisle = 'CC' ORDER BY rack;

-- Expected: 340 rows (20 bays × 5 levels × 2 positions = 200 for CA, etc.)
-- CC: 34 bays × 5 levels × 2 positions = 340 rows